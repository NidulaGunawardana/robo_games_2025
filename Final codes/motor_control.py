...
class CameraControl:
    ...
    def get_depth_and_rgb(self):
        ...
        depth = frame  # Placeholder as depth data is not available from a standard laptop camera
        rgb = frame
...
class RobotNavigator:
    ...
    def process_range_finder(self):
        ...
        depth_image = self.camera_control.get_depth_and_rgb()[0]  # Placeholder for actual depth data
        
        # Convert to RGBA for display
        rgba = cv2.cvtColor(depth_image, cv2.COLOR_GRAY2RGBA)
        # rgba = cv2.normalize(rgba, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)
        
        # Display image using OpenCV
        cv2.imshow("Range Finder View", rgba)
        # cv2.waitKey(0)  # Add a small delay to allow the frame to render
    ...
    def process_camera(self, _depth_image):
        ...
        """
        rgb_image = self.camera_control.get_depth_and_rgb()[1]  # Placeholder for actual RGB data
        cam_data = rgb_image
        # if not cam_data:
        #     return
        ...
        cv2.waitKey(1)  # Add a small delay to allow the frame to render
